...
function ModelDetailSkeleton({ backLabel, from }: { backLabel: string; from: string }) {
  return (
    <div className="model-detail">
      ...
      <div className="model-detail-console-shell model-detail-console-shell--loading" aria-hidden="true">
...
function ModelDetailPageContent() {
    ...
    return (
      <div className="model-detail">
        ...
        <div className="model-detail-console-shell">
          <div className="model-detail-console-empty">{t("modelDetail.notFound")}</div>
        </div>
      </div>
    );
  }
  ...
  const providerName = providerDisplayLabel(currentModel.provider, currentModel.provider_display, locale, t);
  ...
  const pipelineSlotLabel = formatPipelineSlot(currentModel.pipeline_slot) || t("modelDetail.notDeclared");
  const codeValue = currentModel.canonical_model_id || currentModel.model_id;
  const capabilityRows = [
    { label: t("modelDetail.inputModalities"), items: currentModel.input_modalities ?? [] },
    { label: t("modelDetail.outputModalities"), items: currentModel.output_modalities ?? [] },
    { label: t("modelDetail.supportedTools"), items: currentModel.supported_tools ?? [] },
    { label: t("modelDetail.supportedFeatures"), items: currentModel.supported_features ?? [] },
  ];
  const summaryStats = [
    { label: t("modelDetail.inputModalities"), value: currentModel.input_modalities?.length ?? 0 },
    { label: t("modelDetail.outputModalities"), value: currentModel.output_modalities?.length ?? 0 },
    { label: t("modelDetail.supportedTools"), value: currentModel.supported_tools?.length ?? 0 },
    { label: t("modelDetail.aliases"), value: currentModel.aliases?.length ?? 0 },
  ];
  ...
  const metaRows = [
    ...
    { label: t("modelDetail.categoryLabel"), value: categoryName || t("modelDetail.notDeclared") },
    { label: t("modelDetail.groupLabel"), value: groupName || t("modelDetail.notDeclared") },
  ...
  function handleUseModel() {
    if (currentModel.is_selectable_in_console === false || typeof window === "undefined" || !pickerCategory) {
      return;
    }
      ...
      <div className="model-detail-console-shell">
        <div className="model-detail-console-head">
          <div className="model-detail-console-head-copy">
            ...
            <h1 className="model-detail-console-title">{currentModel.display_name}</h1>
            <p className="model-detail-console-provider model-detail-provider">{providerName}</p>
          </div>
        ...
        <div className="model-detail-console-codebar">
          <div className="model-detail-console-codegroup">
            <span className="model-detail-console-field-label">{t("modelDetail.modelCode")}</span>
            <div className="model-detail-console-codefield">{codeValue}</div>
          </div>
          <div className="model-detail-console-badges model-detail-tags">
            {categoryName ? (
              <span className="model-detail-console-chip model-card-tag highlight is-strong">{categoryName}</span>
            ) : null}
            {groupName ? <span className="model-detail-console-chip model-card-tag">{groupName}</span> : null}
            <span className="model-detail-console-chip model-card-tag">{pipelineSlotLabel}</span>
            <span className="model-detail-console-chip model-card-tag model-detail-status is-status">{statusLabel}</span>
          </div>
        </div>

        <div className="model-detail-console-statstrip">
          {summaryStats.map((stat, index) => (
            <div key={stat.label} className={`model-detail-console-stat${index > 0 ? " has-divider" : ""}`}>
        ...
        <div className="model-detail-console-layout">
              ...
              <div className="model-detail-console-rowlist">
                {capabilityRows.map((row) => (
                  <div key={row.label} className="model-detail-console-row">
                    <div className="model-detail-console-rowlabel">{row.label}</div>
                    <div className="model-detail-console-rowvalue">
                      {row.items.length ? row.items.map((item) => (
                        <span key={item} className="model-detail-console-valuepill">
                          <CheckIcon />
                          {labelForToken(item, t)}
                        </span>
                      )) : (
                        <span className="model-detail-console-muted">{t("modelDetail.notDeclared")}</span>
                      )}
                    </div>
                  </div>
                ))}
              ...
              <section className="model-detail-console-section">
                ...
                <div className="model-detail-console-tokenrow">
                  {currentModel.aliases.map((alias) => (
                    <span key={alias} className="model-detail-console-token is-code">
                      {alias}
                    </span>
                  ))}
                </div>
          ...
          <aside className="model-detail-console-side">
              ...
              <section className="model-detail-console-section">
                ...
                <div className="model-detail-console-meta">
                  <div className="model-detail-console-metaitem">
                    ...
                    <strong>{formatPipelineSlot(pickerCategory, t) || t("modelDetail.notDeclared")}</strong>
                  </div>
                  <div className="model-detail-console-metaitem">
                    <span>{t("discover.pickerModel")}</span>
                    <strong>{currentModelId || t("modelDetail.notDeclared")}</strong>
                  </div>
                </div>
              </section>
            ) : null}
            ...
            <section className="model-detail-console-section">
              ...
              <div className="model-detail-console-meta">
                {metaRows.map((row) => (
                  <div key={row.label} className="model-detail-console-metaitem">
                    <span>{row.label}</span>
                    <strong className={row.code ? "is-code" : undefined}>{row.value}</strong>
                  </div>
                ))}
              </div>
              {currentModel.official_url ? (
