...
export default function KnowledgeDetailPage() {
  ...
  const [snapshotMessage, setSnapshotMessage] = useState("v1 baseline");
      ...
      <PageTransition>
        <div className="p-6 space-y-6">
          <div className="console-page-header">
            <h1 className="console-page-title">{t("detail.title")}</h1>
            <p className="console-page-desc">{t("detail.description")}</p>
          </div>
              ...
              <div className="console-panel-body">
                <div className="console-form-grid columns-3">
                  <div className="md:col-span-2">
                    <label className="console-label" htmlFor="snapshot-message">{t("detail.snapshotLabel")}</label>
                    <input
                      id="snapshot-message"
                    ...
                    <button
                      ...
                      onClick={async () => {
                        const res = await apiPost<{ dataset_version: { id: string; version: number } }>(
                          ...
                          { commit_message: snapshotMessage, freeze_filter: { tag: null } },
                        );
                        setSnapshotResult(t("detail.snapshotted", { version: res.dataset_version.version }));
                        setHasNewUploads(false);
                        setSnapshotMessage(`v${res.dataset_version.version + 1}`);
                        await load();
                      }}
                    >
                      {t("detail.snapshotButton")}
                    </button>
                  </div>
                </div>
                {snapshotResult ? <div className="console-inline-notice is-success mt-4">{snapshotResult}</div> : null}
              </div>
            </div>
          ...
          {viewMode === "table" ? (
            <DataTable
              caption={t("itemTable.caption")}
              headers={[t("itemTable.file"), t("itemTable.type"), t("itemTable.size"), t("itemTable.tags"), t("itemTable.actions")]}
              emptyTitle={t("itemTable.emptyTitle")}
              emptyBody={t("itemTable.emptyBody")}
              rows={items.map((item) => [
                <div key={item.id}>
            ...
            <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {items.length ? (
                ...
                <div className="console-panel sm:col-span-2 lg:col-span-3">
                  <div className="console-panel-body">
                    <div className="console-empty">{t("itemTable.emptyTitle")}</div>
                  </div>
                </div>
              )}
            </section>
