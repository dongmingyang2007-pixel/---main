...
OUT = Path("/Users/dog/Desktop/铭润/output/debug_v5/qihang_product_pearl_phase2.glb")
...
def make_patch(name: str, verts, material):
    ...
    local_verts = [Vector(v) for v in verts]
    center = sum(local_verts, Vector()) / len(local_verts)
    center_index = len(local_verts)
    all_verts = local_verts + [center]
    faces = []
    for i in range(len(local_verts)):
        j = (i + 1) % len(local_verts)
...
def main():
    ...
    make_patch("Case_Base_Shell_Patch_Upper", UPPER, shell_material)
    make_patch("Case_Base_Shell_Patch_Lower", LOWER, shell_material)
